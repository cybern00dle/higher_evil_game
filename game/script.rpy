...
label run:
    ...
    "???" "Сынок, ты просто устал. Мало ли, что может с такими кругами под глазами почудиться."
...
label enter:
    ...
    "{i}Мне стало не по себе, начала кружиться голова…{/i}"
...
label good_ending:
    ...
    "{i}Он спокойно открыл ключами ранее запертую дверь и распахнул её перед моим носом.{/i}"
